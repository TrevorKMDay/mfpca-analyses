...
dat_mfpca <- data_mfpca_transformed(df_var = list(microb, metab),
                                    shared_id_unique = 'ID_unique',
                                    shared_time_var = 'Timepoint',
                                    y_var = c('Avg_Shannon_microb',
                                              'Avg_Shannon_metab'),
                                    covariates = c('Sex', 'BodySite'),
                                    keep_all_obs = FALSE, min_visit = 0,
                                    nseed = 31)
...
i <- 1
set.seed(i)
param_stan <- input_mfpca_stan(dat_mfpca = dat_mfpca,
                               num_PCs = mdl_list[[i]]$num_PCs,
                               nknots = mdl_list[[i]]$nknots, nseed = i)

pca_data <- list(N = param_stan$num_subjects,
                  P = param_stan$num_blocks,
                  K = param_stan$num_PCs,
                  Q = param_stan$num_basis,
                  cov_size = param_stan$cov_size,
                  cov_starts = param_stan$cov_starts,
                  cov_stops = param_stan$cov_stops,
                  M = param_stan$num_visits_all,
                  subject_starts = param_stan$subject_starts,
                  subject_stops = param_stan$subject_stops,
				          subject_starts_each = param_stan$subject_starts_each,
				          subject_stops_each = param_stan$subject_stops_each,
				          V = param_stan$visits_matrix, B = param_stan$spline_basis,
				          Y = param_stan$response_vector,
				          len_Theta = param_stan$len_Theta,
				          Theta_idx = param_stan$Theta_idx,
				          Q_idx = param_stan$Q_idx_Theta,
...
fit = sampling(smod, data = pca_data, iter = Nsamples, chains = Nchains,
               cores = Ncores)
